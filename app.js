...
class NotesApp {
  constructor() {
    ...
    this.initElements();
    this.initDB();               // Initialize DB first
    this.loadSettings();         // Then apply saved settings
    this.addEventListeners();
    this.setupObservers();
  }

  initElements() {
    // Map IDs to camelCase properties
    const map = {
      sidebar: 'sidebar',
  ...
  initDB() {
    const request = indexedDB.open('ModernNotesDB', 1);
    request.onerror = e => this.showToast(`DB error: ${e.target.error}`, 'error');
  ...
  loadSettings() {
    ...
    const intervalSec = parseInt(localStorage.getItem('versioningInterval'), 10);
    if (!isNaN(intervalSec)) {
      this.versioningInterval = intervalSec * 1000;
      this.versionFrequency.value = intervalSec;
    }
  ...
  addEventListeners() {
    ...
    document.addEventListener('keydown', e => {
      ...
      if (e.ctrlKey && e.key.toLowerCase() === 'n') {
        e.preventDefault(); this.createNote();
      }
    });
  }
  ...
  loadDataFromDB() {
    ...
    notesTx.getAll().onsuccess = e => {
      this.notes = e.target.result.map(fnote => ({
        ...fnote,
        versions: Array.isArray(fnote.versions)
          ? fnote.versions
          : [{content: fnote.content, date: fnote.date}]
      }));
  ...
  addTag(noteId,tag){
    const note=this.notes.find(n=>n.id===noteId); if(!note) return; note.tags=note.tags||[];
    if(!note.tags.includes(tag)){ note.tags.push(tag); this.tags.add(tag);
      this.saveToIndexedDB('notes',note)
        .then(()=>this.eventEmitter.emit('noteUpdated',note))
        .catch(err=>this.showToast(`Error:${err}`,'error')); }
  }
  ...
  addProject(name){ if(!name) return; const p={id:Date.now(),name,color:this.projectColors[Math.floor(Math.random()*this.projectColors.length)]};
  ...
  deleteProject(id){
    this.deleteFromIndexedDB('projects',id)
      ...
      .then(()=>{this.notes=this.notes.filter(n=>n.projectId!==id);if(this.currentProject===id)this.currentProject=null;
        this.eventEmitter.emit('projectDeleted',id);this.showToast('Proj & notes deleted','success');
      })
      .catch(err=>this.showToast(`Error:${err}`,'error'));
  }

  showVersionHistory(noteId){
    const note=this.notes.find(n=>n.id===noteId); if(!note) return;
    this.versionList.innerHTML='';
    note.versions.forEach((v,i)=>{
      const d=document.createElement('div'); d.className='version-item';
      d.innerHTML=`<div>${i+1}. ${v.date}</div><button class='preview'>Preview</button><button class='restore'>Restore</button>`;
      d.querySelector('.preview').addEventListener('click',()=>this.previewVersion(noteId,i));
      d.querySelector('.restore').addEventListener('click',()=>this.restoreVersion(noteId,i));
      this.versionList.appendChild(d);
    });
    this.versionModal.classList.add('active');
  }

  previewVersion(noteId,i){
    const note=this.notes.find(n=>n.id===noteId); if(!note||!note.versions[i]) return;
    this.versionModal.classList.remove('active');
    const m=document.createElement('div'); m.className='modal active';
    m.innerHTML=`<div class='modal-content'><button class='modal-close'>&times;</button><div>${note.versions[i].content}</div><button class='restore'>Restore</button></div>`;
    m.querySelector('.modal-close').addEventListener('click',()=>{document.body.removeChild(m);this.versionModal.classList.add('active');});
    m.querySelector('.restore').addEventListener('click',()=>{document.body.removeChild(m);this.restoreVersion(noteId,i);});
    document.body.appendChild(m);
  }

  restoreVersion(noteId,i){
    const note=this.notes.find(n=>n.id===noteId); if(!note||!note.versions[i]) return;
    const now=new Date().toLocaleString(); note.versions.push({content:note.content,date:now});
    note.content=note.versions[i].content; note.date=now;
    this.saveToIndexedDB('notes',note)
      .then(()=>{this.eventEmitter.emit('noteUpdated',note);this.closeModals();this.showToast('Restored','success');})
      .catch(err=>this.showToast(`Error:${err}`,'error'));
  }
  ...
  showProjectsModal(){
    ...
    this.projects.forEach(p=>{
      const c=document.createElement('div'); c.className='project-card'; c.style.setProperty('--project-color',p.color);
      c.innerHTML=`<div>${p.name}</div><div>${this.notes.filter(n=>n.projectId===p.id).length} notes</div>`;
      c.addEventListener('click',()=>{this.currentProject=p.id;this.renderNotes();this.closeModals();this.showToast(`Switched to ${p.name}`,'info');});
      this.projectsGrid.appendChild(c);
    });
  ...
  renderTags(){this.tagsContainer.innerHTML='';[...this.tags].sort().forEach(tag=>{const el=document.createElement('div');el.className=`tag ${this.currentTag===tag?'active':''}`;el.textContent=tag;el.addEventListener('click',()=>this.filterByTag(tag));this.tagsContainer.appendChild(el);});}

  renderNotes(){
    this.notesContainer.innerHTML='';
    let out=[...this.notes];
    if(this.currentProject) out=out.filter(n=>n.projectId===this.currentProject);
    if(this.currentTag) out=out.filter(n=>n.tags&&n.tags.includes(this.currentTag));
    if(this.searchTerm) out=out.filter(n=>n.title.toLowerCase().includes(this.searchTerm)||n.content.toLowerCase().includes(this.searchTerm));
    out.sort((a,b)=>new Date(b.date)-new Date(a.date));
    if(!out.length){this.notesContainer.innerHTML='<div class="empty-state"><h3>No notes</h3></div>';return;}
    out.forEach(n=>{
      const el=document.createElement('div');el.className='note';el.dataset.id=n.id;
      el.innerHTML=`<input class='note-title' value='${n.title}'><div class='controls'><button class='versions'>${n.versions.length}</button><button class='add-tag'>+Tag</button><button class='delete'>Del</button></div><div class='note-content' contenteditable>${n.content}</div><div class='footer'>By ${n.author} • ${n.date}</div>`;
      el.querySelector('.note-title').addEventListener('change',e=>this.updateNote(n.id,'title',e.target.value));
      let to;const ce=el.querySelector('.note-content');ce.addEventListener('input',()=>{clearTimeout(to);to=setTimeout(()=>this.updateNote(n.id,'content',ce.innerHTML),this.versioningInterval);});
      el.querySelector('.versions').addEventListener('click',()=>this.showVersionHistory(n.id));
      el.querySelector('.add-tag').addEventListener('click',()=>this.showInputToast('Tag:',t=>this.addTag(n.id,t)));
      el.querySelector('.delete').addEventListener('click',()=>this.deleteNote(n.id));
  ...
  updateWordCount(){
    ...
    this.wordCountElement.textContent=`${cnt} word${cnt!==1?'s':''}`;
    this.statsCountElement.textContent=`${this.notes.length} note${this.notes.length!==1?'s':''}`;
  }
  ...
  showInputToast(msg,cb){
    ...
    const inp=t.querySelector('input');const btn=t.querySelector('button');
    btn.addEventListener('click',()=>{cb(inp.value);t.remove();});
    this.toastContainer.appendChild(t);
  }
}
...
NotesApp.prototype.exportNoteAsHTML = function(noteId) {
  ...
  if(!note){this.showToast('Not found','error');return;}
  const project=this.projects.find(p=>p.id===note.projectId);
  const projectName=project?project.name:'No Project';
  const html=`<!DOCTYPE html>...FULL HTML HERE...`;
  const blob=new Blob([html],{type:'text/html'});
  const url=URL.createObjectURL(blob);
  const a=document.createElement('a');a.href=url;a.download=`${note.title.replace(/[^a-z0-9]/gi,'_').toLowerCase()}.html`;a.click();URL.revokeObjectURL(url);
