...
class NotesApp {
   ...
   * Logs warnings instead of throwing errors for missing elements
   */
  initElements() {
    const map = {
      sidebar: 'sidebar',
    ...
    for (const [id, prop] of Object.entries(map)) {
      ...
      if (!el) {
        console.warn(`Warning: Element #${id} not found; related features may not work.`);
        continue;
      }
      this[prop] = el;
    }
    // Optional modal-close buttons
  ...
  renderTags() {
    ...
    Array.from(this.tags).sort().forEach(tag => {
      ...
      tagEl.className = `tag ${this.currentTag === tag ? 'active' : ''}`;
      tagEl.textContent = tag;
      tagEl.addEventListener('click', () => this.eventEmitter.emit('tagSelected', tag));
  ...
  showInputToast(msg, cb) {
    // Create a toast-style input prompt
    ...
    t.innerHTML = `
        ...
        <div class="toast-content">
            ...
            <input id="toast-input" type="text" placeholder="Type here...">
            <button id="toast-ok">OK</button>
        </div>
    `;
    const inp = t.querySelector('#toast-input');
    const btn = t.querySelector('#toast-ok');
    ...
    inp.focus();
}</div><input><button>OK</button>`;const inp=t.querySelector('input');const btn=t.querySelector('button');btn.addEventListener('click',()=>{cb(inp.value);t.remove();});this.toastContainer.appendChild(t);}  
}
