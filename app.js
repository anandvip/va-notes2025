...
class EventEmitter {
  ...
  emit(evt, data) { (this.events[evt]||[]).forEach(fn=>{try{fn(data)}catch(e){console.error(evt, e)}}); }
  off(evt, fn) { if(!this.events[evt]) return; this.events[evt] = fn ? this.events[evt].filter(f=>f!==fn) : []; }
}

class NotesApp {
  constructor() {
    // State
    this.db = null; this.notes = []; this.projects = [];
    this.tags = new Set(); this.currentProject = null; this.currentTag = null;
    this.searchTerm = '';
  ...
  initElements() {
    ...
    ids.forEach(id => {
      ...
      else this[id.replace(/-([a-z])/g, (_,c)=>c.toUpperCase())] = el;
    });
  ...
  initDB() {
    ...
    req.onupgradeneeded = e=>{
      ...
      if (!this.db.objectStoreNames.contains('notes')) this.db.createObjectStore('notes',{keyPath:'id'});
      if (!this.db.objectStoreNames.contains('projects')) this.db.createObjectStore('projects',{keyPath:'id'});
    };
  ...
  loadSettings() {
    ...
    if(!isNaN(sec)){ this.versioningInterval=sec; this.versionFrequency.value=sec; }
  ...
  addProject(name){ if(!name) return; const p={id:Date.now(),name,color:this.projectColors[Math.floor(Math.random()*8)]}; this.projects.unshift(p); this.db.transaction('projects','readwrite').objectStore('projects').put(p); this.eventEmitter.emit('projectAdded',p); this.showToast(`Project "${name}" added`,'success'); }
  deleteProject(id){
  ...
  removeTag(id,tag){ const note=this.notes.find(n=>n.id===id); note.tags=n.tags.filter(t=>t!==tag); this.db.transaction('notes','readwrite').objectStore('notes').put(note); this.eventEmitter.emit('noteUpdated',note); }
  ...
  renderNotes(){ this.notesContainer.innerHTML=''; let list=this.notes.slice(); if(this.currentProject) list=list.filter(n=>n.projectId===this.currentProject); if(this.currentTag) list=list.filter(n=>n.tags.includes(this.currentTag)); if(this.searchTerm) list=list.filter(n=>n.title.toLowerCase().includes(this.searchTerm)||n.content.toLowerCase().includes(this.searchTerm)); list.sort((a,b)=>new Date(b.date)-new Date(a.date)); if(!list.length){ this.notesContainer.innerHTML='<div class="empty-state"><h3>No notes</h3></div>'; return; } list.forEach(n=>{ const el=document.createElement('div'); el.className='note'; el.dataset.id=n.id; el.innerHTML=`
  <div class="note-header">
    <input class="note-title" value="${n.title}" />
    <button class="delete">Del</button>
  </div>
  <div class="note-content" contenteditable="true">${n.content}</div>
  <div class="note-footer">${n.author} • ${n.date}</div>
`; el.querySelector('.note-title').onchange=e=>this.updateNote(n.id,'title',e.target.value); el.querySelector('.note-content').addEventListener('input',e=>{ clearTimeout(this._deb); this._deb=setTimeout(()=>this.updateNote(n.id,'content',e.target.innerHTML),this.versioningInterval); }); el.querySelector('.delete').onclick=()=>this.deleteNote(n.id); this.notesContainer.append(el); }); this.updateWordCount(); }
  ...
  // Modals, formatting, export omitted for brevity

  // Toast
  showToast(msg,type='info',dur=3000){ const t=document.createElement('div'); t.className=`toast ${type}`; t.textContent=msg; this.toastContainer.append(t); setTimeout(()=>t.remove(),dur); }

  // Prompt
  showInputToast(msg,cb){ const t=document.createElement('div'); t.className='toast info'; t.innerHTML=`<div>${msg}</div><input /><button>OK</button>`; const inp=t.querySelector('input'); t.querySelector('button').onclick=()=>{cb(inp.value); t.remove();}; this.toastContainer.append(t); }
}
